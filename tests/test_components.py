...
def relational_db(session):
    ...
    relational_db = session.relational_db
    saved_config = {"verbose": relational_db.verbose,
...
def test_relational(relational_db):
	...
	assert relational_db.test_connection(print_results=True), "Basic tests on relational database connection failed."

@pytest.mark.order(2)
def test_sql_examples(relational_db):
	""" """
	_test_sql_file(relational_db, "./tests/reset.sql", expect_df=False)
	_test_sql_file(relational_db, "./tests/example1.sql", expect_df=True, df_header="EntityName table:")
	_test_sql_file(relational_db, "./tests/example2.sql", expect_df=True, df_header="ExampleEAV table:")
	df = relational_db.get_dataframe("EntityName")  ## Internal errors are handled by the class itself.
	assert df is not None   ## We can just check results since implementation is checked by RelationalConnector.

def _test_sql_file(relational_db, filename: str, expect_df: bool, df_header: str = ""):
	"""Run queries from a local file through t
	...
	try:
		...
		if expect_df:
			assert df is not None, f"Execution of '{filename}' failed to produce results."
			if df_header:
				print(df_header)
				print(df)
	except:
