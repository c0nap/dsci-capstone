...
# Note: Most code in this file has been generated by ChatGPT.


# Not implemented
class Chunk:
	def __init__(self, text: str, story_id: int, segment_id: int,
				 story_percent: float, segment_percent: float,
				 line_start: int, line_end: int, max_chunk_length: int = -1):
		self.text :int = text
		self.story_id :int = story_id
		self.segment_id :int = segment_id
		self.story_percent :float = story_percent
		self.segment_percent :float = segment_percent
		self.line_start :int = line_start
		self.line_end :int = line_end
		self.max_chunk_length :int = max_chunk_length
		self.length = self.char_count(False)
		if self.length > max_chunk_length:
			raise f"Invalid chunk: Text has {self.length} chars, but expected fewer than {max_chunk_length}"
...
class StoryStreamAdapter(ABC):
	...
	def stream_paragraphs(self) -> Chunk:
		...
		The Chunk class is repurposed here so we pass location info.
		But depending on the Story.pre_split_chunks implementation, this might be unnecessary.
		"""
		pass
...
class Story:
	...
	def pre_split_chunks(self, max_chunk_length :int):
		"""
		Creates an indexer to keep track of where each chunk starts.
		Chunks should stick to paragraphs when possible, but if a paragraph is insanely long
		or if the story has no paragraphs, it's acceptable to split by sentence.
		Ideally we would use the LangChain tokenizer here to maximize chunk size.
		This is a classic problem: minimize the number of chunks while sticking to discrete paragraphs.
		We start by fetching the length of the segment using Chunk.char_count, then
		use max_chunk_length to estimate chunks, and stream paragraphs to refine borders.
...
class Book1:
	...
	def _create_chunk(self, text: str, line_start: int, line_end: int,
		...
		return Chunk(
			...
			line_end=line_end,
			story_percent=(total_book_chars / self.total_book_chars) * 100 if self.total_book_chars > 0 else 0,
			segment_percent=(chapter_chars / total_chapter_chars) * 100 if total_chapter_chars > 0 else 0
		)